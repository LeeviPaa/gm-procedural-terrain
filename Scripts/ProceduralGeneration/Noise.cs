...
namespace Procedural
{
    ...
    {
        public static NoiseSample PerlinSample(Vector2 point, float scale, float frequency)
        {
            point = point / scale;
            return Perlin2D(point, frequency);
        }

        public static NoiseSample PerlinSample2(Vector2 point, float scale, float frequency)
        {
            point = point / scale * frequency;
            return new NoiseSample(Mathf.PerlinNoise(point.x, point.y), Vector3.zero);
        }
	    ...
	    public static NoiseSample Perlin2D (Vector2 point, float frequency) 
        {
	    	...
	    	NoiseSample sample;
	    	sample.Height = a + b * tx + (c + d * tx) * ty;
	    ...
	    public static NoiseSample Perlin3D (Vector3 point, float frequency) 
        {
	    	...
	    	NoiseSample sample;
	    	sample.Height = a + b * tx + (c + e * tx) * ty + (d + f * tx + (g + h * tx) * ty) * tz;
